...
\begin{abstract}
We formulate the behavior of 1-point functions in the source-free,
symmetric vacuum of an exact conformal theory, taking
$\AdS_5 \times S^5$ and its boundary dual $\N=4$ $SU(N)$ super
Yang--Mills theory as the reference system.
As candidates for detecting nontrivial 1-point vacuum data
we adopt the operator family
$\Cex = \bigl\{\mathcal{O}_\Delta^a(x),\,
J_\mu^A(x),\, T_{\mu\nu}(x),\,
\partial^\mu J_\mu^A(x)\bigr\}$,
where $\mathcal{O}_\Delta^a$ are local scalar primaries with
$\Delta>0$, $J_\mu^A$ are conserved currents, and $T_{\mu\nu}$ is
the energy-momentum tensor.

As the first result, we prove a unified vanishing theorem:
in the Poincar\'e-invariant, conformally invariant, source-free
vacuum of $\N=4$ SYM, the 1-point functions of all operators
in $\Cex$ vanish.
The individual vanishing results---via boundary Ward identities
and bulk holographic renormalization---are separately well known;
the unified formulation for the operator family~$\Cex$ provides
the basis for the systematic analysis that follows.
Second, under deformations
$S = S_{\mathrm{CFT}} + \Delta S_{\mathrm{def}}$---relevant
deformations, finite temperature, finite chemical potential,
vacuum selection, time-dependent backgrounds, etc.---we systematize
the conditions under which $\Cex$-relative nonzero 1-point functions
are permitted, and identify the deformations for which
constructive realizations are known.
The $\Lambda>0$ background is placed in this framework as
a geometric implementation of such deformations.

Third, we examine observables outside $\Cex$---defects
(Wilson loops, etc.), information-theoretic quantities
(entanglement entropy, mutual information), and higher-point
correlators---and classify observables into four layers.
The vanishing theorem is specific to Layer~I (local 1-point data);
in the standard example, Layers~II--IV carry nontrivial data
even in the symmetric vacuum (scope-delimitation theorem).
Fourth, we unify the four-layer classification with the
deformation theory and show that representative deformations
trigger systematic responses across all four layers
...
The arguments throughout rely on established results in quantum
field theory and the AdS/CFT correspondence;
the individual ingredients are separately known.
What is new are structural facts that emerge only from their
unification:
the vanishing is sharply confined to Layer~I,
the deformation response across all four layers is systematic
rather than ad hoc, and the sole unconfirmed cell
($\Lambda>0 \times$ Layer~II) is precisely locatable.
All theorems are independent of the completeness of~$\Cex$.
What remains open is not the correctness of the theorems but
the completeness of the adopted operator family~$\Cex$ and
its four-layer extension---that is, the \emph{scope} of the
claims made here.
...
mass generation, or the selection of an ordered phase;
...
invariance used on flat Minkowski space is lost.

\begin{proposition}\label{prop:time-dep}
Under a time-dependent background
$g^{(0)}_{\mu\nu}(x)$ or time-dependent
...
dual~$\N=4$~SYM\@.
In Section~\ref{sec:3} we organized how this $\Cex$-relative
vanishing theorem is lifted by the introduction of a
deformation~$\Delta S_{\mathrm{def}}$.
...
assumptions supporting the $\Cex$-relative vanishing theorem of
Section~\ref{sec:2} are lost.
...
defined not via an AdS-type boundary S-matrix or Euclidean
...
supporting the $\Cex$-relative vanishing theorem of
Section~\ref{sec:2}: the flat-space implementation of scale
...
The $\Cex$-relative vanishing theorem for 1-point functions in
the source-free, symmetric vacuum established in
Section~\ref{sec:2} for $\AdS_5\times S^5/\N=4$~SYM does not hold
...
\begin{remark}\label{rem:unification-scope}
Theorem~\ref{thm:unification} is $\Cex$-relative
(see \S\ref{subsec:1-5} and \S\ref{subsec:2-7}).
\end{remark}
...
$\Cex$-relative vanishing theorem of Section~\ref{sec:2} are
not preserved on a de~Sitter background.
\end{proof}
...
\begin{remark}\label{rem:final-scope}
This theorem is $\Cex$-relative
(see \S\ref{subsec:1-5} and \S\ref{subsec:2-7}).
\end{remark}
...
This is understood as a geometric implementation of the finite-temperature response of \S\ref{subsec:7-3}: in
...
In Section~\ref{sec:5} we assembled the $\Cex$-relative
...
$\Cex$-relative nontrivial 1-point data as two boxed
propositions.
...
\texttt{02\_script/sec6\_wilson\_loop.py}.
...
\noindent\textbf{Script:}
\texttt{02\_script/sec7\_relevant\_defect.py}.
...
\noindent\textbf{Script:}
\texttt{02\_script/sec7\_finite\_density\_info.py}.
...
\noindent\textbf{Script:}
\texttt{02\_script/sec7\_finite\_density\_corr.py}.
...
\colrule
{[Comp.~1]} & \texttt{02\_script/sec6\_wilson\_loop.py}
  & \texttt{03\_results/sec6\_wilson\_loop.json} \\
{[Comp.~2]} & \texttt{02\_script/sec7\_relevant\_defect.py}
  & \texttt{03\_results/sec7\_relevant\_defect.json} \\
{[Comp.~3]} & \texttt{02\_script/sec7\_finite\_density\_info.py}
  & \texttt{03\_results/sec7\_finite\_density\_info.json} \\
{[Comp.~4]} & \texttt{02\_script/sec7\_finite\_density\_corr.py}
  & \texttt{03\_results/sec7\_finite\_density\_corr.json} \\
\end{tabular}
\end{ruledtabular}
\end{table}
